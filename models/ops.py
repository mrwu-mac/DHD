...
def associate_with_ground_truth(boxes_h, boxes_o, targets, num_classes, fg_iou_thresh):
        n = boxes_h.shape[0]
        labels = torch.zeros(n, num_classes, device=boxes_h.device)

        gt_bx_h = recover_boxes(targets['boxes_h'], targets['size'])
        gt_bx_o = recover_boxes(targets['boxes_o'], targets['size'])

        x, y = torch.nonzero(torch.min(
            box_iou(boxes_h, gt_bx_h),
            box_iou(boxes_o, gt_bx_o)
        ) >= fg_iou_thresh).unbind(1)
