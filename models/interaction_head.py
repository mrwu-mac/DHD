...
from models.ops import compute_spatial_encodings

import random
from hicodet.static_hico import HICO_INTERACTIONS, ACT_TO_ING, TRAIN_VERB_NUMS, HOI_IDX_TO_OBJ_IDX, TRAIN_HOI_NUMS, \
                        HOI_IDX_TO_OBJ_IDX, HOI_IDX_TO_ACT_IDX, RARE_HOI_IDX, NON_RARE_HOI_IDX, UO_HOI_IDX, OBJ_IDX_TO_OBJ_NAME, NEW_HOI, HOI_TO_NEW_VERB
...
class InteractionHead(nn.Module):
    ...
    def __init__(self,
        box_pair_predictor: nn.Module,
        binary_predictor: nn.Module,
        hidden_state_size: int, representation_size: int, teacher_dim: int,
        ...
        #         nn.Linear(hidden_state_size + teacher_dim, hidden_state_size),
        #         nn.ReLU(),
        #         nn.Linear(hidden_state_size, hidden_state_size),
        #         nn.ReLU()
        #     )
        # self.global_head = nn.Sequential(
        #         nn.Linear(num_channels + teacher_dim, num_channels),
        #         nn.ReLU(),
        ...
        self.clip_map = nn.Sequential(nn.Linear(representation_size * 2, teacher_dim),
                                      nn.ReLU())
        self.clip_map_u = nn.Sequential(nn.Linear(hidden_state_size, teacher_dim),
                                      nn.ReLU())
        self.embedding = nn.Parameter(torch.zeros(1, hidden_state_size))
        self.text_map = nn.Linear(512, hidden_state_size)
        self.unseen_objs = torch.as_tensor(HOI_IDX_TO_OBJ_IDX)[UO_HOI_IDX].unique()
        
        # print([OBJ_IDX_TO_OBJ_NAME[ii] for ii in self.unseen_objs])
        # self.tmp_map = {OBJ_IDX_TO_OBJ_NAME.index('cat'):OBJ_IDX_TO_OBJ_NAME.index('dog'), 
        #                 OBJ_IDX_TO_OBJ_NAME.index('sheep'):OBJ_IDX_TO_OBJ_NAME.index('elephant'),
        #                 OBJ_IDX_TO_OBJ_NAME.index('horse'):OBJ_IDX_TO_OBJ_NAME.index('zebra'),
        #                 OBJ_IDX_TO_OBJ_NAME.index('sports_ball'):OBJ_IDX_TO_OBJ_NAME.index('frisbee'),
        #                 OBJ_IDX_TO_OBJ_NAME.index('snowboard'):OBJ_IDX_TO_OBJ_NAME.index('skis'),
        #                 OBJ_IDX_TO_OBJ_NAME.index('sandwich'):OBJ_IDX_TO_OBJ_NAME.index('pizza'),
        #                 OBJ_IDX_TO_OBJ_NAME.index('microwave'):OBJ_IDX_TO_OBJ_NAME.index('refrigerator'),
                        
        #                 OBJ_IDX_TO_OBJ_NAME.index('surfboard'):OBJ_IDX_TO_OBJ_NAME.index('skateboard')}
        # self.head_predictor = head_predictor
        # self.tail_predictor = tail_predictor
        # self.head_predictor.weight.data = self.box_pair_predictor.weight[NON_RARE_HOI_IDX, :].data
        # self.tail_predictor.weight.data = self.box_pair_predictor.weight[RARE_HOI_IDX, :].data
    ...
    def forward(self, image_shapes: Tensor, region_props: List[dict], box_features: Tensor, global_features: Tensor, obj_text_features: Tensor, targets, text_features, verb_text_features):
        """
        Parameters:
        -----------
        ...
        # global_features_detr = self.avg_pool(features).flatten(start_dim=1)
        # global_features = self.global_head(torch.cat([global_features, global_features_detr], 1))
        ...
        for b_idx, props in enumerate(region_props):
            ...
            scores = props['scores']
            labels = props['labels']
            # unary_tokens_detr = props['hidden_states']
            # print(box_features[b_idx].shape)
            unary_tokens = self.box_head(box_features[b_idx])
            # unary_tokens = torch.cat([unary_tokens, unary_tokens_detr], 1)
            # unary_tokens = self.unary_head(unary_tokens)
           
            # # if self.training:
            # #     # print(labels, self.tmp_map.keys())
            # #     new_boxes = []
            # #     new_scores = []
            # #     new_labels = []
            # #     new_tokens = []
            # #     for iii, label in enumerate(labels):
            # #         if label.item() in self.tmp_map.keys():
            # #             # print('do')
            # #             new_boxes.append(boxes[iii])
            # #             new_scores.append(scores[iii])
            # #             new_labels.append(self.tmp_map[label.item()])
            # #             new_tokens.append(self.text_map(obj_text_features[self.tmp_map[label.item()]]) + self.embedding)
                
            # #     if len(new_boxes):
            # #         boxes = torch.cat([boxes, torch.stack(new_boxes)])
            # #         scores = torch.cat([scores, torch.as_tensor(new_scores, device=boxes.device)])
            # #         labels = torch.cat([labels, torch.as_tensor(new_labels, device=boxes.device, dtype=labels.dtype)])
            # #                 # print(unary_tokens[iii].shape, obj_text_features.shape, self.embedding.shape)
            # #         unary_tokens = torch.cat([unary_tokens, torch.cat(new_tokens)])
            # if self.training and self.cl:
            #     new_boxes = []
            #     new_scores = []
            #     new_labels = []
            #     new_tokens = []
            #     target_hois = targets[b_idx]['hoi']
            #     objs = []
            #     weights = []
            #     for hoi in target_hois:
            #         objs.append([OBJ_IDX_TO_OBJ_NAME.index(p[1].replace(' ', '_')) for p in NEW_HOI[HOI_TO_NEW_VERB[hoi]]])
            #         weight = []
            #         for obj in objs[-1]:
            #             if obj in labels:
            #                 weight.append(0.5)
            #             elif obj in self.unseen_objs:
            #                 weight.append(2)
            #             else:
            #                 weight.append(1.0)
            #         weights.append(weight)
            #     # print('ori:',[OBJ_IDX_TO_OBJ_NAME[ii] for ii in labels])
            #     # objs = np.unique(objs)
                    
            #     for iii, label in enumerate(labels):
            #         for iiii, objss in enumerate(objs):
            #             if label.item() in objss:
            #                 # new_label = random.choices(objss, weights=weights[iiii], k=1)[0]
            #                 new_label = label
            #                 # print('do')
            ...
            #                 new_tokens.append(self.text_map(obj_text_features[new_label]) + self.embedding)
            #                 break
            #     # print('new:',[OBJ_IDX_TO_OBJ_NAME[ii] for ii in new_labels])
            #     if len(new_boxes):
            #         boxes = torch.cat([boxes, torch.stack(new_boxes)])
            #         scores = torch.cat([scores, torch.as_tensor(new_scores, device=boxes.device)])
            #         labels = torch.cat([labels, torch.as_tensor(new_labels, device=boxes.device, dtype=labels.dtype)])
            ...
            if not torch.all(labels[:n_h]==self.human_idx):
                ...
                box_features = box_features[b_idx][perm]
            ...
            if n_h == 0 or n <= 1:
                ...
                unary_tokens_collated.append(torch.zeros(
            ...
            # unary_tokens_collated.append(unary_tokens[x_keep])
            # unary_tokens_collated.append(unary_tokens[y_keep])
            unary_tokens_collated.append(unary_tokens[x_keep])
            # pre_verb = self.avg_pool(box_features[b_idx]).flatten(start_dim=1)[x_keep] @ verb_text_features.T
            # print(torch.max(pre_verb, dim=-1))
            # print(targets[b_idx]['verb'])
            # boxes_collated.append(boxes)
        ...
        pairwise_tokens_collated = torch.cat(pairwise_tokens_collated)  # (n, d_model)
        unary_tokens_collated = torch.cat(unary_tokens_collated)

        # pair_hs = self.pair_embed(pairwise_tokens_collated)
        # unary_hs = self.unary_embed(unary_tokens_collated)
        # logits = self.box_pair_predictor(pairwise_tokens_collated)  # (n, 117)
        binary_logits = self.binary_predictor(pairwise_tokens_collated)
        embed = self.clip_map(pairwise_tokens_collated)
        # logits = self.box_pair_predictor(pairwise_tokens_collated)
        embed = embed / embed.norm(dim=-1, keepdim=True)
        logits = self.scale.exp() * embed @ text_features.T


        u_embed = self.clip_map_u(unary_tokens_collated)
        u_embed = u_embed / u_embed.norm(dim=-1, keepdim=True)
        verb_logits = self.scale.exp() * u_embed @ verb_text_features.T


        return binary_logits, logits, verb_logits, prior_collated, \
            boxes_h_collated, boxes_o_collated, object_class_collated, attn_maps_collated, boxes_collated
