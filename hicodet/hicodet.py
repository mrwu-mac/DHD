...
from .static_hico import UA_HOI_IDX, UC_HOI_IDX, RARE_HOI_IDX, NON_RARE_HOI_IDX, UO_HOI_IDX, UV_HOI_IDX
...
class HICODet(ImageDataset):
    ...
    def __init__(self, root: str, anno_file: str,
            ...
            transforms: Optional[Callable] = None, training=True, training_type='full') -> None:
        ...
        self.unseen_idx = []
        if training_type == 'ua':self.unseen_idx=UA_HOI_IDX
        if training_type == 'uv':self.unseen_idx=UV_HOI_IDX
        if training_type == 'uo':self.unseen_idx=UO_HOI_IDX
        if training_type == 'rfuc':self.unseen_idx=UC_HOI_IDX['rare_first']
        if training_type == 'nfuc':self.unseen_idx=UC_HOI_IDX['non_rare_first']
        if training_type in ['uc0', 'uc1', 'uc2', 'uc3', 'uc4']:self.unseen_idx=UC_HOI_IDX[training_type]
        # if training_type == 'rare':self.unseen_idx=NON_RARE_HOI_IDX
        if training_type == 'nonrare':self.unseen_idx=RARE_HOI_IDX
    ...
    def _load_annotation_and_metadata(self, f: dict) -> None:
        ...
        for empty_idx in np.unique(f['empty']):
            idx.remove(empty_idx)

        num_anno = [0 for _ in range(self.num_interation_cls)]
        for ii, anno in enumerate(f['annotation']):
            for i, hoi in enumerate(anno['hoi']):
                num_anno[hoi] += 1

                ### del unseen label
            #     if self.training and hoi in self.unseen_idx:
            #         for k in anno.keys():
            #             f['annotation'][ii][k].pop(i)
            # if self.training and len(anno['hoi']) == 0 and ii in idx:
            #     idx.remove(ii)

                if self.training and hoi in self.unseen_idx:
                    if ii in idx:idx.remove(ii)
                    break
