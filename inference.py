...
from models.dhd import build_detector
...
def visualise_entire_image(image, output, actions, action=None, thresh=0.2):
    ...
    # pred = output['labels']
    scores = output['bi_scores']
    objects = output['bi_objects']
    pred = output['bi_labels']
    ...
    if action is not None:
        ...
        bx_h, bx_o = boxes[output['bi_pairing']].unbind(0)
...
def main(args):
    ...
    dataset = DataFactory(name=args.dataset, partition=args.partition, data_root=args.data_root, clip_model=args.clip_model)
    ...
    dhd = build_detector(args, conversion)
    dhd.eval()

    if os.path.exists(args.resume):
        print(f"=> Continue from saved checkpoint {args.resume}")
        checkpoint = torch.load(args.resume, map_location='cpu')
        dhd.load_state_dict(checkpoint['model_state_dict'])
    else:
