...
def crop(image, target, region):
    ...
    if "boxes_g" in target:
        boxes = target["boxes_g"]
        max_size = torch.as_tensor([w, h], dtype=torch.float32)
        cropped_boxes = boxes - torch.as_tensor([j, i, j, i])
        cropped_boxes = torch.min(cropped_boxes.reshape(-1, 2, 2), max_size)
        cropped_boxes = cropped_boxes.clamp(min=0)
        area = (cropped_boxes[:, 1, :] - cropped_boxes[:, 0, :]).prod(dim=1)
        target["boxes_g"] = cropped_boxes.reshape(-1, 4)
        fields_g.append("boxes_g")
    ...
    if "boxes" in target or "masks" in target or "boxes_h" in target or "boxes_o" in target or "boxes_g" in target:
        ...
        if "boxes_g" in target:
            cropped_boxes = target['boxes_g'].reshape(-1, 2, 2)
            keep_g = torch.all(cropped_boxes[:, 1, :] > cropped_boxes[:, 0, :], dim=1)

        for field in fields:
            target[field] = target[field][keep]
        for field in fields_g:
            if field in target:
                target[field] = target[field][keep_g]
